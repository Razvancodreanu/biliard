...
export class UI {
    constructor(canvas) {
        ...
        this.aiming = false; this.aimStart = null; this.aimMouse = null; this.power = 0;
        // events
        canvas.addEventListener('mousedown', e => this._down(e));
        window.addEventListener('mousemove', e => this._move(e));
        window.addEventListener('mouseup', e => this._up(e));
        this.onPointerDown = null; this.onPointerDrag = null; this.onPointerUp = null;
    }
    getMouse(e) {
    ...
    updateAim(m) {
        ...
        const L = Math.hypot(dx, dy); const maxL = 220; this.power = Math.min(1, L / maxL);
    }
    endAim(m, p) {
        if (!this.aiming) return { fired: false };
        const dx = m.x - this.aimStart.x, dy = m.y - this.aimStart.y;
        this.aiming = false; const power = this.power; this.power = 0; this.aimStart = null; this.aimMouse = null;
